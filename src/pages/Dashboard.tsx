...
/*** =====================
 * Dashboard (restored UI)
 * - Keeps previous nicer layout (stats row, probation card w/ legend, device health table, top branches, recent logs 10)
 * - Adds persistent probation dismissal + server ack
 * - Normalizes manager branch scoping (name/slug/id) across all sections
 * ===================== */

/** Device is considered stale/offline after this many minutes */
const OFFLINE_MINUTES = 5
/** Show probation within this window around day-90 */
const OVERDUE_WINDOW_DAYS = 3   // include up to 3 days past 90
const UPCOMING_WINDOW_DAYS = 7  // include up to 7 days before 90
/** Persist reviewed/dismissed probation items here */
...
type ProbationRow = Employee & { probationDue?: boolean; probationStatus?: string; daysToProbation?: number }
...
export default function Dashboard() {
  ...
  function tokenHeader() {
    ...
    return t ? { Authorization: /^bearer/i.test(t) ? t : `Bearer ${t}` } : {}
  }
  ...
  // ---- helpers: probation normalization + window filter ----
  const fmtDate = (v: any) => {
    if (!v) return ''
    const s = String(v)
  ...
  /** Make a stable key for an employee across shapes (id→uid→code→name+branch→name+joined→name) */
  ...
  function includeInWindow(d: any) { if (!Number.isFinite(d)) return false; const n = Number(d); return n >= -OVERDUE_WINDOW_DAYS && n <= UPCOMING_WINDOW_DAYS }

  function findEmployeeMatch(source: Employee[], probe: any): Employee | undefined {
    const pid = probe?.id ?? probe?.employee_id ?? probe?.emp_id
    if (pid != null) { const byId = source.find(e => String(e.id) === String(pid)); if (byId) return byId }
    const pcode = probe?.code ?? probe?.employee_code
    if (pcode) { const byCode = source.find(e => String((e as any).code || '') === String(pcode)); if (byCode) return byCode }
    const puid = probe?.uid ?? probe?.employee_uid
    if (puid) { const byUid = source.find(e => String((e as any).uid || '') === String(puid)); if (byUid) return byUid }
    const pname = normalizeNameKey(probe?.name ?? probe?.employee?.name)
    if (pname) { const byName = source.find(e => normalizeNameKey(e.name) === pname); if (byName) return byName }
    return undefined
  }
  function normalizeProbationItems(items: any[], sourceEmployees: Employee[]): ProbationRow[] {
    return (items || []).map((x: any) => {
      const emp = x?.employee || x?.user || x?.emp || x
      const match = findEmployeeMatch(sourceEmployees, x) || findEmployeeMatch(sourceEmployees, emp) || emp
      ...
      const uid = match?.uid ?? emp?.uid ?? x?.uid ?? ''
      const code = match?.code ?? emp?.code ?? x?.code ?? ''
      const joined_at = match?.joined_at ?? emp?.joined_at ?? x?.joined_at ?? null

      let d90 = x?.daysToProbation ?? x?.days_to_probation ?? x?.days_to_90 ?? x?.until_90
      if (!Number.isFinite(d90)) d90 = daysTo90(joined_at)

      const status = x?.probationStatus ?? x?.status ?? (Number.isFinite(d90) ? (d90 <= 0 ? 'Overdue' : (d90 <= 3 ? 'Soon' : 'upcoming')) : '-')

      return {
        ...(match as any),
        id: id as any,
        name, branch, uid, code, joined_at,
  ...
  function localProbationFromEmployees(list: Employee[]): ProbationRow[] {
    return list.map((e) => {
      ...
      return { ...(e as any), daysToProbation: d90, probationStatus: d90 <= 0 ? 'Overdue' : (d90 <= 3 ? 'Soon' : 'upcoming') } as ProbationRow
    }).filter(Boolean) as ProbationRow[]
  }

  // --- DISMISS STORAGE ---
  function readDismissed(): Record<string, string> { try { const raw = localStorage.getItem(DISMISS_STORE_KEY); return raw ? JSON.parse(raw) : {} } catch { return {} } }
  function writeDismissed(map: Record<string, string>) { try { localStorage.setItem(DISMISS_STORE_KEY, JSON.stringify(map)) } catch {} }
  function markDismissedFor(rowLike: any, resolved?: Employee | null) {
    const map = readDismissed()
    const name = normalizeNameKey(rowLike?.name)
    const branch = normalizeBranchKey(rowLike?.branch)
    const joined = normDateKey(rowLike?.joined_at)

    const rid = resolved?.id ?? rowLike?.id
    const ruid = resolved?.uid ?? rowLike?.uid
    const rcode = resolved?.code ?? rowLike?.code
    const rname = normalizeNameKey(resolved?.name ?? rowLike?.name)
    ...
    const keys = new Set<string>([
      rid != null ? `id:${rid}` : '', ruid ? `uid:${ruid}` : '', rcode ? `code:${rcode}` : '',
      rname && rbranch ? `nb:${rname}|${rbranch}` : '', rname && rjoined ? `nj:${rname}|${rjoined}` : '', rname ? `n:${rname}` : '', rbranch ? `b:${rbranch}` : '',
      rowLike?.id != null ? `id:${rowLike.id}` : '', rowLike?.uid ? `uid:${rowLike.uid}` : '', rowLike?.code ? `code:${rowLike.code}` : '',
      name && branch ? `nb:${name}|${branch}` : '', name && joined ? `nj:${name}|${joined}` : '', name ? `n:${name}` : '', branch ? `b:${branch}` : '',
    ].filter(Boolean) as string[])

    if (keys.size) { const ts = new Date().toISOString(); keys.forEach(k => { map[k] = ts }); writeDismissed(map) }
  }
  function isDismissed(row: any, resolved?: Employee | null): boolean {
    const map = readDismissed()
    const rid = resolved?.id ?? row?.id
    const ruid = resolved?.uid ?? row?.uid
    const rcode = resolved?.code ?? row?.code
    const rname = normalizeNameKey(resolved?.name ?? row?.name)
    ...
    const candidates = [ rid != null ? `id:${rid}` : '', ruid ? `uid:${ruid}` : '', rcode ? `code:${rcode}` : '', rname && rbranch ? `nb:${rname}|${rbranch}` : '', rname && rjoined ? `nj:${rname}|${rjoined}` : '', rname ? `n:${rname}` : '', rbranch ? `b:${rbranch}` : '' ].filter(Boolean) as string[]
  ...
  function gcDismissedAgainst(list: ProbationRow[]) {
    ...
    for (const r of list) {
      ...
      ;[ rid != null ? `id:${rid}` : '', ruid ? `uid:${ruid}` : '', rcode ? `code:${rcode}` : '', rname && rbranch ? `nb:${rname}|${rbranch}` : '', rname && rjoined ? `nj:${rname}|${rjoined}` : '', rname ? `n:${rname}` : '', rbranch ? `b:${rbranch}` : '' ].filter(Boolean).forEach(k => { if (map[k]) keep.add(k) })
    }
    if (keep.size === 0) return
    const next: Record<string, string> = {}; keep.forEach(k => { next[k] = readDismissed()[k] }); writeDismissed(next)
  }

  // ---- Manager scoping helpers ----
  function normalizeBranchKey(raw:any) {
    if (raw == null) return ''
    const s = String(typeof raw === 'object' ? (raw.name ?? raw.title ?? raw.slug ?? raw.branch ?? raw) : raw).trim().toLowerCase()
    return s.replace(/\s+/g, '')
  }
  function extractBranchInfo(x:any): { nameKey: string; idKey: string } {
    const b = x?.branch ?? x?.deviceBranch ?? x?.branch_name ?? x?.branchName ?? (typeof x?.branch === 'object' ? (x.branch.name ?? x.branch.title) : undefined)
  ...
  const allowedSets = useMemo(() => normalizeAllowed(allowedBranchesRaw), [JSON.stringify(allowedBranchesRaw)])
  ...
  async function load() {
    try {
      ...
      const [l, e, d, p] = await Promise.all([
        ...
        api.getDevices?.() ?? Promise.resolve([]),
        api.getProbationDue?.() ?? Promise.resolve({ items: [] as any[] }),
      ])
      ...
      const serverP = normalizeProbationItems((p as any).items || p || [], eScoped)
      const localP  = localProbationFromEmployees(eScoped)
      setProbation(mergeServerAndLocal(serverP, localP, eScoped))
    } catch (err: any) {
      setError(err?.message ?? 'Failed to load dashboard')
    }
  }

  function mergeServerAndLocal(serverRows: ProbationRow[], localRows: ProbationRow[], empList: Employee[]) {
    const serverKeys = new Set<string>(serverRows.map(computeKeyLike))
    const out: ProbationRow[] = []

    function maybePush(r: ProbationRow) {
      if (!r) return
    ...
    localRows.forEach((r) => { const k = computeKeyLike(r); if (k && serverKeys.has(k)) return; maybePush(r) })

    out.sort((a, b) => { const da = Number.isFinite(a.daysToProbation!) ? (a.daysToProbation as number) : 9999; const db = Number.isFinite(b.daysToProbation!) ? (b.daysToProbation as number) : 9999; return da - db })
    gcDismissedAgainst(out)
    return out
  }

  useEffect(() => { load() }, [])
  useEffect(() => { if (me) { load() } // eslint-disable-next-line
  }, [me?.role, JSON.stringify(allowedBranchesRaw)])

  // --- Derived stats ---
  const deviceStats = useMemo(() => {
    const now = Date.now(), cut = OFFLINE_MINUTES * 60 * 1000
    let total = devices.length, online = 0, offline = 0, needs = 0
  ...
  const branchesCount = useMemo(() => new Set((employees as any[]).map(e => String(e.branch || ''))).size, [employees])
  const branchTotals = useMemo(() => {
    const by: Record<string, number> = {}
    for (const e of employees as any[]) { const b = String(e.branch || ''); if (!b) continue; by[b] = (by[b] || 0) + 1 }
    return Object.entries(by).sort((a,b) => b[1]-a[1]).slice(0, 8)
  }, [employees])

  // Probation small stats for legend bar widths
  const probStats = useMemo(() => {
    ...
    const soon = probation.filter(a => { const d = a.daysToProbation ?? 0; return d > 0 && d <= 3 }).length
  ...
  const barWidths = useMemo(() => {
    ...
    return {
      ...
      rest: Math.max(0, 100 - Math.round((probStats.overdue / n) * 100) - Math.round((probStats.soon / n) * 100)),
    }
  }, [probation, probStats])
  ...
  async function onReviewFirst() {
    ...
    const id = resolved?.id ?? first?.id
    if (id != null) { try { await api.ackProbation(id) } catch {} }
    markDismissedFor({ id:first.id, uid:first.uid, code:first.code, name:first.name, branch:first.branch, joined_at:first.joined_at }, resolved)
    setProbation(prev => prev.filter(r => computeKeyLike(r) !== computeKeyLike(resolved || first)))
    openReviewCard(first)
  }

  // ---- Edit modal (re-uses old UI; still read-only for manager) ----
  ...
  function openReviewCard(row?: ProbationRow) {
    ...
    setEditing({
      ...
      employment_type: (match as any)?.employment_type ?? (target as any)?.employment_type ?? null,
  ...
  async function saveEdit(e: React.FormEvent) {
    e.preventDefault()
    if (ro) { alert('Read-only role: managers cannot edit employees.'); return }
    if (!editing) return
    const real = findEmployeeMatch(employees, editing) || editing
    const id = (real as any)?.id ?? (editing as any)?.id
    if (id == null) { alert('Could not resolve employee ID.'); return }

    try {
      const payload = { ...(editing as any), id }
      await api.updateEmployee(id, payload)
      try { await api.ackProbation(id) } catch {}
      const resolved = findEmployeeMatch(employees, payload) || payload
      markDismissedFor(payload, resolved as any)
      setProbation(prev => prev.filter(r => computeKeyLike(r) !== computeKeyLike(resolved)))
      setShowEdit(false)
    } catch (err:any) { alert(err?.message || 'Failed to save employee') }
  }
  ...
  const recentRows = (logs as any[]).map((r:any, i:number) => {
    ...
    return {
      ...
      code: emp.code || r.code || '',
      branch: r.branch || emp.branch || dev.branch || dev.branch_name || '',
      device: (dev.name || r.device_name || `Device ${dev.id ?? ''}`) + (dev.branch ? ` (${dev.branch})` : (dev.branch_name ? ` (${dev.branch_name})` : '')),
    }
  })
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <StatCard title="Branches" value={branchesCount} />
        <StatCard 
      ...
      {/* Probation + Device Health side by side */}
      <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4 mt-3">
                <div className="flex w-48 h-2 rounded-full overflow-hidden bg-muted">
                  ...
                  <div className="h-2 bg-muted-foreground/30" style={{ width: `${barWidths.rest}%` }} />
                </div>
        ...
        <div className="card">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${deviceStats.offline > 0 ? 'bg-yellow-500' : 'bg-green-500'}`}></div>
                    ...
                    return (
                      <tr key={String(d.id || i)} className="hover:bg-muted/50 transition-colors border-b border-border/50">
                        ...
                        <td className="px-2 py-3">
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${isOn ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300'}`}>
                            {isOn ? 'Online' : 'Offline'}
                          </span>
                        </td>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 max-w-20 h-2 bg-muted rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${Math.max(10, (c / Math.max(...branchTotals.map(([,count]) => count))) * 100)}%` }}
                        />
                      </div>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <form onSubmit={saveEdit} className="w-full max-w-xl rounded-2xl bg-white dark:bg-zinc-900 p-4 space-y-3 shadow-xl">
            <div className="flex items-start justify-between">
              ...
              <button type="button" onClick={() => setShowEdit(false)} className="text-sm text-zinc-500">Close</button>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <Field label="Name"><input className="input" disabled={ro} value={(editing as any).name || ''} onChange={e=>setEditing({...(editing as any), name:e.target.value})}/></Field>
              <Field label="Branch"><input className="input" disabled={ro} value={(editing as any).branch || ''} onChange={e=>setEditing({...(editing as any), branch:e.target.value})}/></Field>
              <Field label="Department"><input className="input" disabled={ro} value={(editing as any).department || ''} onChange={e=>setEditing({...(editing as any), department:e.target.value})}/></Field>
              <Field label="Phone"><input className="input" disabled={ro} value={(editing as any).phone || ''} onChange={e=>setEditing({...(editing as any), phone:e.target.value})}/></Field>
              <Field label="UID"><input className="input" disabled={ro} value={(editing as any).uid || ''} onChange={e=>setEditing({...(editing as any), uid:e.target.value})}/></Field>
              <Field label="Code"><input className="input" disabled={ro} value={(editing as any).code || ''} onChange={e=>setEditing({...(editing as any), code:e.target.value})}/></Field>
              <Field label="Employment Type">
                <select className="input" disabled={ro} value={(editing as any).employment_type || ''} onChange={e=>{
              ...
              {(editing as any).employment_type === 'wages' && (
                <Field label="Hourly rate"><input className="input" type="number" step="0.01" disabled={ro} value={(editing as any).hourly_rate ?? ''} onChange={e=>setEditing({...(editing as any), hourly_rate: Number(e.target.value)})}/></Field>
              )}
              {(editing as any).employment_type === 'salary' && (
                <Field label="Salary (IQD)"><input className="input" type="number" step="1" disabled={ro} value={(editing as any).salary_iqd ?? ''} onChange={e=>setEditing({...(editing as any), salary_iqd: Number(e.target.value)})}/></Field>
              )}
              <Field label="Joined at"><input className="input" type="date" disabled={ro} value={fmtDate((editing as any).joined_at)} onChange={e=>setEditing({...(editing as any), joined_at: e.target.value || null} as any)}/></Field>
            </div>
...
function Field({label, children}:{label:string, children:React.ReactNode}) {
  return <label className="space-y-1 block"><span className="text-xs">{label}</span>{children}</label>
}
...
function StatCard({
  ...
  return (
    <div className={`rounded-2xl bg-zinc-900/5 dark:bg-zinc-800 p-6 shadow-sm card border hover:shadow-md transition-shadow ${status ? statusColors[status] : 'border-border/50'}`}>
...
function Chip({ label, value, color = 'blue' }:{label:string; value:React.ReactNode; color?:'blue'|'green'|'red'|'yellow'}) {
  const colors = {
